...
namespace Altinn.Register.Tests.Utils
{
    public static class PrincipalUtil
    {
        ...
        public static string GetToken(int userId, int authenticationLevel = 2)
        {
            ...
            claims.Add(new Claim(AltinnCoreClaimTypes.PartyUUID, Guid.ParseExact(userId.ToString("D32"), "N").ToString("D"), ClaimValueTypes.String, issuer));
        ...
        public static string GetServiceOwnerOrgToken(string org, int authenticationLevel = 4)
        {
            List<Claim> claims = new List<Claim>();
            string issuer = "www.altinn.no";
