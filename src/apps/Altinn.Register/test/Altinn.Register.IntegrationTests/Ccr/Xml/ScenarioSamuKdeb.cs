...
/// <summary>
/// SAMU KDEB: Removes the debtor/company in liquidation (konkursdebitor) role of an organization.
/// </summary>
public class ScenarioSamuKdeb
    : CcrXmlUpdateTestBase
{
    ...
    protected override async ValueTask Setup(IUnitOfWork uow, CancellationToken cancellationToken)
    {
        ...
        _personKDEB = await uow.CreatePerson(
            name: PersonName.Create("Forrige", "konkursdebitor"),
            cancellationToken: cancellationToken);
        ...
        await uow.AddRole(ExternalRoleSource.CentralCoordinatingRegister, roleIdentifier: "konkursdebitor", from: _org.PartyUuid.Value, to: _personKDEB.PartyUuid.Value, cancellationToken);
        await uow.AddRole(ExternalRoleSource.CentralCoordinatingRegister, roleIdentifier: "konkursdebitor", from: _org.PartyUuid.Value, to: _orgKDEB.PartyUuid.Value, cancellationToken);
    }
        ...
        <batchAjourholdXML xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="batchAjourholdXML_versjon2_1.xsd">
          ...
          <enhet organisasjonsnummer="{{_org.OrganizationIdentifier.Value}}" organisasjonsform="ESEK" hovedsakstype="E" undersakstype="EN" foersteOverfoering="N" datoFoedt="20130413" datoSistEndret="20260504">
            <samendringUtgaar felttype="SAMU">
                <samendringstype>KDEB</samendringstype>
            </samendringUtgaar>
          </enhet>
