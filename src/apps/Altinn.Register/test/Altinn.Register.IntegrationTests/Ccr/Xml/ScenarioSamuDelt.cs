...
/// <summary>
/// SAMU DELT: Removes all participants (deltaker) of an organization, including those with full responsibility (deltaker-fullt-ansvar) and shared responsibility (deltaker-delt-ansvar).
/// </summary>
public class ScenarioSamuDelt
    : CcrXmlUpdateTestBase
{
    ...
    protected override async ValueTask Setup(IUnitOfWork uow, CancellationToken cancellationToken)
    {
        ...
        _personMedlOld1 = await uow.CreatePerson(
            name: PersonName.Create("Forrige", "Deltaker 1"),
            cancellationToken: cancellationToken);

        _personMedlOld2 = await uow.CreatePerson(
            name: PersonName.Create("Forrige", "Deltaker 2"),
            cancellationToken: cancellationToken);
        ...
        await uow.AddRole(ExternalRoleSource.CentralCoordinatingRegister, roleIdentifier: "deltaker-fullt-ansvar", from: _org.PartyUuid.Value, to: _personMedlOld1.PartyUuid.Value, cancellationToken);
        await uow.AddRole(ExternalRoleSource.CentralCoordinatingRegister, roleIdentifier: "deltaker-delt-ansvar", from: _org.PartyUuid.Value, to: _personMedlOld2.PartyUuid.Value, cancellationToken);
    }
        ...
        <batchAjourholdXML xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="batchAjourholdXML_versjon2_1.xsd">
          ...
          <enhet organisasjonsnummer="{{_org.OrganizationIdentifier.Value}}" organisasjonsform="ESEK" hovedsakstype="E" undersakstype="EN" foersteOverfoering="N" datoFoedt="20130413" datoSistEndret="20260504">
            <samendringUtgaar felttype="SAMU">
                <samendringstype>DELT</samendringstype>
            </samendringUtgaar>
          </enhet>
