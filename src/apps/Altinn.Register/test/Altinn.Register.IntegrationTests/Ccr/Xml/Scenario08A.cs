    ...
    : CcrXmlUpdateTestBase
{
    ...
    protected override string XmlToApply
        ...
        <batchAjourholdXML xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="batchAjourholdXML_versjon2_1.xsd">
          ...
          <enhet organisasjonsnummer="{{_org.OrganizationIdentifier.Value}}" organisasjonsform="ESEK" hovedsakstype="E" undersakstype="EN" foersteOverfoering="N" datoFoedt="20210508" datoSistEndret="20260504">
            <infotype felttype="FADR" endringstype="N">
              <postnr>1234</postnr>
              <landkode>NO</landkode>
              <kommunenr>0301</kommunenr>
              <adresse1>c/o Test Testperson</adresse1>
              <adresse2>Testveien 29</adresse2>
            </infotype>
          </enhet>
    ...
    protected override async ValueTask Verify(IUnitOfWork uow, CancellationToken cancellationToken)
    {
        ...
        updatedOrg?.UnitType.Value.ShouldBe("ESEK");
        updatedOrg?.BusinessAddress.Value?.PostalCode.ShouldBe("1234");
        updatedOrg?.BusinessAddress.Value?.Address?.Contains("Testveien 29").ShouldBeTrue();
        updatedOrg?.BusinessAddress.Value?.Address?.Contains("c/o Test Testperson").ShouldBeTrue();
    }
}
