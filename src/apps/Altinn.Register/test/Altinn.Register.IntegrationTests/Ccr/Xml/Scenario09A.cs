...
/// <summary>
/// Adds two new "regnskapsforer" roles to the same organization, where there already exists one "regnskapsforer" role.
/// One of the two new roles is added in the same XML batch, the other is a check for idempotency.
/// </summary>
public class Scenario09A
    : CcrXmlUpdateTestBase
{
    ...
    protected override async ValueTask Setup(IUnitOfWork uow, CancellationToken cancellationToken)
    {
        ...
        await uow.AddRole(
            ...
            from: _org.PartyUuid.Value,
            to: _regn1.PartyUuid.Value,
            cancellationToken);
    }
    ...
    protected override string XmlToApply
        ...
        <batchAjourholdXML xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="batchAjourholdXML_versjon2_1.xsd">
          ...
          <enhet organisasjonsnummer="{{_org.OrganizationIdentifier.Value}}" organisasjonsform="AS" hovedsakstype="E" undersakstype="EN" foersteOverfoering="N" datoFoedt="20210611" datoSistEndret="20260503">
            <samendringer data="D" felttype="REGN" endringstype="N" type="K">
              ...
              <knytningRekkefoelge>4</knytningRekkefoelge>
