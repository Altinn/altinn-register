...
/// <summary>
/// SAMU DAGL: Removes the general manager (daglig-leder) role of an organization.
/// </summary>
public class ScenarioSamuDagl
    : CcrXmlUpdateTestBase
{
    ...
    protected override async ValueTask Setup(IUnitOfWork uow, CancellationToken cancellationToken)
    {
        ...
        _personDAGL = await uow.CreatePerson(
            name: PersonName.Create("Forrige", "daglig-leder"),
            cancellationToken: cancellationToken);
