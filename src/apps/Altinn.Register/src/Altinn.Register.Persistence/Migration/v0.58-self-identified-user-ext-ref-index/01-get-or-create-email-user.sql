CREATE OR REPLACE FUNCTION register.get_or_create_email_user(
  ...
  OUT p_display_name register.party.display_name%TYPE,
  ...
  OUT p_self_identified_ext_ref register.self_identified_user.ext_ref%TYPE,
...
BEGIN
  ...
  WHERE si.email = p_self_identified_email
  LIMIT 1;
    ...
    BEGIN
      SELECT *
        FROM register.upsert_self_identified_user(
          ...
          TRUE, p_self_identified_email, -- display_name
          ...
          TRUE, NULL, -- owner
          'idporten-email', -- self_identified_user_type
          TRUE, p_self_identified_email,
          TRUE, NULL) -- self_identified_ext_ref
        INTO
          p_uuid,
          p_id,
          p_ext_urn,
      ...
      EXCEPTION
        WHEN unique_violation THEN
          -- Another transaction created a user with the same email concurrently.
            SELECT p, si, u
            INTO o_result
            FROM register.self_identified_user si
            JOIN register.party p USING ("uuid")
